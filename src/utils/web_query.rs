...
pub trait QueryGetTrait{
    ...
    fn get_parse<T>(&self,key:&str)->Result<T>  where T:FromStr,T::Err:std::fmt::Display;
}
impl QueryGetTrait for Query<QueryGet>{
    fn get_string(&self,key:&str)->Result<String>{
        return self.0.iter().fold(Result::Err(ErrorBadRequest(format!("not find {} param",key))), |xx, x|{
            if x.0==key{
                return Result::Ok(x.1.to_string());
            }else{
                return xx;
            }
        });
